...
static void parse_host_port(const char *arg,
                            ...
                            const char *what)
{
    ...
    if (!colon) {
        fprintf(stderr,
                "Invalid %s format (expected host:port)\n",
                what);
        exit(EXIT_FAILURE);
    }
...
void parse_args(int argc, char **argv,
                ...
                char *local_host, size_t local_len,
                int *local_port,
                char *target_host, size_t target_len,
                int *target_port)
{
    int opt;
    ...
    while ((opt = getopt(argc, argv, "n:l:t:")) != -1) {
        ...
        case 'l':
            parse_host_port(optarg,
                            local_host, local_len,
                            local_port,
                            "local address");
            break;
